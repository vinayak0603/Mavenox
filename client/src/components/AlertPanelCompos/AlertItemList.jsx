// components/AlertItemList.jsx
import React from "react";
import { AlertTriangle, Bell, AlertOctagon } from "lucide-react";

// Reusable single alert component
const AlertItem = ({ level, message, timestamp, status, icon, borderColor, textColor }) => {
  return (
    <div className={`flex items-start gap-3 border-l-4 pl-4 py-2 ${borderColor} bg-[#0f172a]`}>
      <div className="pt-1">{icon}</div>
      <div className="w-full">
        <p className={`${textColor} font-semibold text-sm`}>
          [{level}] <span className="text-white">{message}</span>
        </p>
        <p className="text-xs text-gray-400 mt-1">
          Timestamp: {timestamp}
          <span className={`ml-4 ${textColor}`}>Status: {status}</span>
        </p>
      </div>
    </div>
  );
...
// Full alert list + heading component
const AlertItemList = () => {
  const alerts = [
    {
      id: 1,
  ...
  return (
    <div className="bg-[#0f172a] text-white font-mono rounded-lg overflow-hidden border border-[#1e293b] w-full max-w-6xl">
      {/* Header */}
      ...
      <div className="px-4 py-3 border-t border-[#1e293b] text-xs text-gray-400">
        ...
        <p className="text-green-400 mt-1">Alert system status: ACTIVE</p>
      </div>
    </div>
  );
};
